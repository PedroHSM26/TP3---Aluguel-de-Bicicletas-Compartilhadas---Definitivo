...
public class MainController {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (Integer i = 0; i < usr.size(); i++) {
					if (usr.get(i).getEmail().equals(email.getText())
						...
						JOptionPane.showOptionDialog(frame, "Usu�rio logado", "", JOptionPane.PLAIN_MESSAGE,
	...
	public static void showRegister(JFrame frame, LinkedList<Usuario> usr) {
		...
		JRadioButton planoEletrico = new JRadioButton("Plano El�trico");
		...
		cadastrarBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (nome.getText().isEmpty() || email.getText().isEmpty() || password.getText().isEmpty()
					...
					JOptionPane.showOptionDialog(frame, "Voc� n�o pode deixar campos vazios ao realizar o cadastro!",
					...
					if (planoNormal.isSelected()) {
						...
						Usuario usuario = new Usuario(email.getText(), password.getText(), nome.getText(),
								nacionalidade.getText(), "Bicicleta El�trica", telefone.getText(),cpf.getText());
						usr.add(usuario);
					} else if (planoPlus.isSelected()) {
						Usuario usuario = new Usuario(email.getText(), password.getText(), nome.getText(),
								nacionalidade.getText(), "Plus (Normal & El�trica)", telefone.getText(), cpf.getText());
						usr.add(usuario);
					}

					JOptionPane.showOptionDialog(frame, "Usu�rio Criado com Sucesso!", "", JOptionPane.PLAIN_MESSAGE,
							JOptionPane.QUESTION_MESSAGE, null, new Object[] {}, null);
					JComponent comp = (JComponent) e.getSource();
					Window win = SwingUtilities.getWindowAncestor(comp);
					win.dispose();
